  ...
  },
  {
    ...
    body: (
      <>
        <StepList>
          <Step>
            ใช้ช่องค้นหาด้านบน (ค้นหาได้ทั้งชื่อเมนู, หมวดหมู่, ราคา) หรือกดปุ่มกรอง
            &quot;อาหาร/เครื่องดื่ม&quot; และหมวดหมู่ ถ้ารายการเมนูเยอะจนหายาก
          </Step>
          <Step>
  ...
  },
  {
    ...
    body: (
      <StepList>
        ...
        <Step>
          ค้นหาวัตถุดิบได้จากชื่อ, ผู้จำหน่าย หรือต้นทุน — ตั้งชื่อซ้ำกับที่มีอยู่แล้วไม่ได้
          (ระบบจะแจ้งเตือนให้เปลี่ยนชื่อ)
        </Step>
      </StepList>
    ),
  },
    ...
    body: (
      <>
        <StepList>
          ...
          <Step>
            สร้าง &quot;กลุ่มตัวเลือก&quot; (เช่น ท็อปปิ้ง, ระดับความเผ็ด)
            แล้วผูกเข้ากับเมนูที่ต้องการ — ราคา/ต้นทุนของตัวเลือกจะรวมเข้าไปในบิลอัตโนมัติ
          </Step>
          <Step>
            เวลาสร้าง &quot;หมวดหมู่&quot; ใหม่ ต้องเลือก &quot;ประเภท&quot; ด้วยว่าเป็น อาหาร หรือ
            เครื่องดื่ม — ใช้กรองเมนูที่หน้าขายหน้าร้าน (POS) ให้หาเมนูเจอไวขึ้น
          </Step>
          <Step>
            ค้นหาเมนูได้จากชื่อ, หมวดหมู่ หรือราคา — และมี dropdown กรองตามหมวดหมู่แยกต่างหาก
          </Step>
        </StepList>
      </>
    ),
  ...
  },
  {
    ...
    body: (
      <div className="space-y-3">
        <div>
          <p className="font-medium">ลิงก์คำเชิญ (invite) หมดอายุหรือใช้ไม่ได้แล้ว?</p>
          <p className="text-muted-foreground">
            ลิงก์หมดอายุใน 7 วัน ให้เจ้าของร้าน/ผู้จัดการเข้าหน้า &quot;จัดการผู้ใช้&quot;
            แล้วส่งคำเชิญให้อีเมลเดิมใหม่อีกครั้ง
          </p>
        </div>
        <div>
          <p className="font-medium">กด &quot;ยกเลิก (Void)&quot; ไม่ได้ ปุ่มหายไปเฉยๆ?</p>
          <p className="text-muted-foreground">
            แปลว่ารายการนั้นข้ามวันทางธุรกิจไปแล้ว (ดูหัวข้อ &quot;ตั้งค่าระบบ&quot;
            เรื่องชั่วโมงเริ่มวันทางธุรกิจ) ต้องใช้ &quot;ขอคืนเงิน&quot; แทน
            ซึ่งจะไปรออนุมัติที่เมนู &quot;อนุมัติคืนเงิน&quot;
          </p>
        </div>
        <div>
          <p className="font-medium">อัปโหลดสลิปค่าใช้จ่ายแล้วจำนวนเงินที่อ่านได้ผิด?</p>
          <p className="text-muted-foreground">
            ระบบอ่านตัวเลขจากรูปให้อัตโนมัติเพื่อความสะดวกเท่านั้น ไม่ได้แม่นยำ 100%
            (และยังไม่รองรับไฟล์ PDF) ให้ตรวจสอบและแก้ตัวเลขในฟอร์มก่อนกดบันทึกทุกครั้ง
          </p>
        </div>
        <div>
          <p className="font-medium">ขายของได้ทั้งที่ระบบเตือนว่าวัตถุดิบไม่พอ ผิดปกติไหม?</p>
          <p className="text-muted-foreground">
            ไม่ผิดปกติ — ค่าเริ่มต้นของระบบคือเตือนแต่ยังขายต่อได้ (กันขายไม่ทันตอนลูกค้าเยอะ)
            ถ้าอยากให้บล็อกการขายจริงๆ เจ้าของร้านตั้งค่าได้ที่ &quot;ตั้งค่าระบบ&quot; →
            นโยบายสต็อกไม่พอ
          </p>
        </div>
        <div>
          <p className="font-medium">สร้างวัตถุดิบ/เมนู/หมวดหมู่ซ้ำชื่อเดิมไม่ได้?</p>
          <p className="text-muted-foreground">
            ระบบกันชื่อซ้ำในแต่ละประเภทข้อมูลไว้ตั้งใจ (ไม่สนตัวพิมพ์เล็ก-ใหญ่)
            เพื่อไม่ให้เลือกผิดตอนค้นหา/ทำรายการ ถ้าจำเป็นต้องใช้ชื่อคล้ายกันจริงๆ
            ให้เติมรายละเอียดต่อท้าย เช่น ขนาดหรือรุ่น
          </p>
        </div>
        <div>
          <p className="font-medium">ลืมรหัสผ่านเข้าระบบ?</p>
          <p className="text-muted-foreground">
            กด &quot;ลืมรหัสผ่าน?&quot; ที่หน้า Login กรอกอีเมลที่ใช้เข้าระบบ
            ระบบจะส่งลิงก์ตั้งรหัสผ่านใหม่ให้ทางอีเมล
          </p>
        </div>
      </div>
    ),
